...
// This code is basically an async logger in Rust that writes lines to files, 
// but it uses some clever tricks to make it look asynchronous while still 
// writing to disk in a safe way.
...
pub async fn loggin_test() {
    let login_handle = get_handle(&"login.txt");
    let logout = get_handle(&"logout.txt");
    ...
    for name in names {
        ...
        let file_handle_two = logout.clone();
